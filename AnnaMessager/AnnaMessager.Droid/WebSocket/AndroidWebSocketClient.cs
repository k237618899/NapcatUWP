...
namespace AnnaMessager.Droid.WebSocket
{
    public class AndroidWebSocketClient : WebSocketClientBase
    {
        ...
        public override async Task ConnectAsync(Uri uri)
        {
            try
            {
                ...
                foreach (var header in _headers)
                    ...
                    {
                        Debug.WriteLine(
                            $"設置 Android WebSocket 標頭失敗: {header.Key} = {header.Value}, Error: {ex.Message}");
                    }
            ...
            {
                Debug.WriteLine($"Android WebSocket 連接失敗: {ex.Message}");
                _isConnected = false;
        ...
        public override async Task SendAsync(string message)
        {
            ...
            {
                Debug.WriteLine($"Android WebSocket 發送消息失敗: {ex.Message}");
                OnError(new WebSocketErrorEventArgs(ex));
            }
        }
        ...
        public override async Task SendAsync(byte[] data)
        {
            ...
            {
                Debug.WriteLine($"Android WebSocket 發送二進制消息失敗: {ex.Message}");
                OnError(new WebSocketErrorEventArgs(ex));
            }
        }
        ...
        public override async Task CloseAsync(int closeCode = 1000, string reason = "Normal closure")
        {
            if (_webSocket != null && IsConnected)
                ...
                {
                    Debug.WriteLine($"Android WebSocket 關閉失敗: {ex.Message}");
                }
        }
        ...
        private async void ReceiveLoop()
        {
            ...
            while (_webSocket != null && _webSocket.State == WebSocketState.Open &&
                ...
                {
                    Debug.WriteLine($"Android WebSocket 接收消息異常: {ex.Message}");
                    _isConnected = false;
                    OnError(new WebSocketErrorEventArgs(ex));
                    break;
                }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                {
                    Debug.WriteLine($"釋放 Android WebSocket CancellationTokenSource 時發生錯誤: {ex.Message}");
                }
                ...
                if (_webSocket != null)
                    ...
                    {
                        Debug.WriteLine($"釋放 Android WebSocket 資源時發生錯誤: {ex.Message}");
                    }
                    finally
                    {
                        _webSocket = null;
